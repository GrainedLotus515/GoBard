...
GODAVE_DIR="$(go list -m -f '{{.Dir}}' github.com/disgoorg/godave)"
if [ -z "${GODAVE_DIR}" ] || [ ! -d "${GODAVE_DIR}" ]; then
	echo "github.com/disgoorg/godave is not available in the module graph" >&2
	exit 1
fi

NON_INTERACTIVE=1 sh "${GODAVE_DIR}/scripts/libdave_install.sh" "${VERSION}"
